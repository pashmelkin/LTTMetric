...
class GetPRController {
    ...
    get = async (request: express.Request, response: express.Response) => {
        ...
        let {myPulls, error} = await Middleware.GetPullRequests(this.config.owner, this.config.product, "");
        ...
        for (const pr of myPulls) {
            const dates = await Middleware.GetCommitDate ( this.config.owner , this.config.product , pr.id );
